...
export default function AdminInventory() {
    const { user, loading } = useAuth("/admin/login"); // Should ideally check for role='admin' specifically too
    ...
    const handleStatusUpdate = async (id: string, newStatus: string, item: any) => {
        ...
        try {
            ...
            // 3. AUTO-ISSUE REWARD when e-waste is Accepted
            if (newStatus === "Accepted") {
                try {
                    const rewardResponse = await fetch('/api/admin/rewards', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            customer_id: item.user_id,
                            ewaste_request_id: id,
                            reward_type: 'Points',
                            reward_name: `E-Waste Contribution - ${item.ewaste_type}`,
                            reward_value: 500,
                            description: `Thank you for contributing ${item.ewaste_type} to the circular economy!`,
                            status: 'Issued'
                        })
                    });

                    if (!rewardResponse.ok) {
                        console.error('Failed to issue reward:', await rewardResponse.text());
                    }
                } catch (rewardError) {
                    console.error('Reward issuance error:', rewardError);
                }
            }

            // 4. Auto-Migrate to Inventory (Legacy: IF 'Collected')
